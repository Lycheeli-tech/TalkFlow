...
export function AppEntry() {
  ...
  useEffect(() => {
    if (!token) return;
    void getApplicationEntry(token).then(setEntry).catch(() => setError("Unable to load your learning space."));
  }, [token]);

  if (!token) return <OnboardingFlow />;
  if (!entry && !error) return <main className="page-shell"><p className="notice" role="status">Loading FluentLoop…</p></main>;
...
export function AppShell({ labels, children }: { labels: Record<string, string>; children: React.ReactNode }) {
  return <main className="app-shell"><nav className="primary-nav" aria-label={labels.label}><Link href="/">{labels.today}</Link><Link href="/practice">{labels.practice}</Link><Link href="/my-english">{labels.myEnglish}</Link><Link href="/journey">{labels.journey}</Link></nav><section className="app-content">{children}</section></main>;
}
