# FluentLoop MVP Build Specification v2.0
...
- 批准日期：2026-09-12；
- 状态：产品负责人已审阅通过，正式生效；
- 审阅输入：已通过的 `docs/FLUENTLOOP_COURSE_CORE_CHANGE_SPEC_v1.0_zh-CN.md`；
...
3. 用户可以打开任意 Course；
4. Course 01–29 各有核心问题和追问；
5. Course 30 只有核心问题；
6. Catalog 始终为 30 Course / 59 Question；
7. AI 不能改变 Question 和回答重点。

### Course 页面与录音

8. Questions 在桌面端位于左侧；
9. 用户可以直接选择追问；
10. 录音期间不能切题或离开；
11. 打开辅助面板不影响录音；
12. 同一时间只播放一个音频；
...
16. 同一 Question 可以连续保存多次独立 Answer；
17. Answer 不创建 Course 完成或 Mastery；
18. Answer 保存后不自动进入追问或下一门 Course；
19. English Transcript 在录音结束后生成；
20. Feedback 失败不丢失 Answer；
21. 中文 Answer 必须先展示中文和英文稿；
22. 中文稿必须经用户确认后保存；
23. 未确认中文 Draft 不进入 History 或 Memory；
24. Transcript 用户不可编辑；
25. History 按 Question 统计。

### 录音与删除

26. 每个用户、Question、语言只保留最近两条录音；
27. 旧录音过期后 Answer 文字和 Feedback 仍保留；
28. 用户删除 Answer 前必须确认；
29. 删除 Answer 会处理 Transcript、Feedback、Audio 和 Memory Source；
30. Storage 删除失败可重试且不破坏 Answer 数据。

### About Me 与 Memory

31. About Me 可以为空；
32. 用户可以保存多个目标岗位和多份 Resume；
33. 用户可以查看和删除全部 Memory；
34. AI 可以判断 CREATE / UPDATE / MERGE / IGNORE；
35. Memory 自动保存但必须通过来源和 ownership 验证；
36. 未通过验证的 Memory 动作不写入数据库；
37. 删除 Answer 时，只删除失去全部来源的 Memory。

### Practice

38. 用户可以选择 3 题或 5 题；
39. Question 从 59 道静态题随机选择且场内不重复；
40. Practice 不显示 Course 帮助；
41. Practice 支持暂停、跳题、返回和重答；
42. 只在整场结束后显示 Feedback；
43. Practice 不提供历史；
44. Practice 不触发 Legacy 状态。

### 安全与 Legacy

45. 新模块不 import 或调用 Legacy UI、Service、Repository；
46. 新路径不创建 Legacy Session 或 Attempt；
47. 新路径不更新 Day、Phase、XP、streak、Mastery 或 Aha；
48. 用户 A 无法读取或修改用户 B 的 Resume、Answer、Audio、Memory 或 Practice Run；
49. AI、STT、TTS 或 Storage 局部失败不会导致已保存 Answer 丢失；
50. Course 11 与其他 Course 使用同一个通用引擎。
